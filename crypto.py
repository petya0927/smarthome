...
def config_plot(ax, coin, currency, current_price, price_lst, time_lst):
    ...
    ax.set_title(f'{coin}/{currency}\nPrice: {current_price} {CURRENCIES[currency]}\nChange: {round(change_percent(price_lst[-2], price_lst[-1]), 2)}%\n24h change: {round(change_percent(price_lst[0], price_lst[-1]), 2)}%')
    ...
    ax.text(len(time_lst), price_lst[-1], str(current_price) + CURRENCIES[currency])
    max_price = max(price_lst)
    min_price = min(price_lst)
    ax.text(price_lst.index(max_price), price_lst[price_lst.index(max_price)], str(max_price) + CURRENCIES[currency])
    ax.text(price_lst.index(min_price), price_lst[price_lst.index(min_price)], str(min_price) + CURRENCIES[currency])
...
def plotting(root, fig, ax, coin, currency):
    ...
    root.after(60000, lambda: plotting(root, fig, ax, coin, currency))

def main(root, coin, currency):
    fig, ax = plt.subplots()
    plot_ui(root, fig)

    plotting(root, fig, ax, coin, currency)
